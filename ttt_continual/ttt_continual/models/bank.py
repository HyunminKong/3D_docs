...
class SkillBank(nn.Module):
    ...
    def __init__(self, layer_shapes, dim_key, n_slots=16, rank=8,
        ...
        # Keys live in the standardised descriptor space, so they are spread at
        # unit scale. Initialising them an order of magnitude smaller puts every
        # slot at nearly the same distance from every query, and the nearest is
        # then decided by noise -- one slot absorbs the whole stream.
        self.keys = nn.Parameter(torch.randn(n_slots, dim_key))
        ...
        # Writes are staged, not applied where they are produced. Keys and
        # factors are ordinary parameters that the retrieval graph is holding on
        # to, and mutating them in place mid-episode makes the backward pass
        # fail on a version-counter mismatch. Consolidation therefore happens
        # once per optimiser step, after the graph is gone.
        self._pending = []
    ...
    def write(self, query, delta, index=None):
        ...
        if self.write_beta <= 0:
            return None
        if index is None:
            index = torch.cdist(query.detach(), self.keys.detach()).argmin(-1)
        self._pending.append((query.detach().clone(),
                              {l: {k: v.detach().clone() for k, v in d.items()}
                               for l, d in delta.items()},
                              index.detach().clone()))
        return index

    @torch.no_grad()
    def flush(self):
        """Apply every staged write. Call after the optimiser step."""
        if not self._pending:
            return 0
        applied = 0
        for query, delta, index in self._pending:
            applied += self._consolidate(query, delta, index)
        self._pending.clear()
        return applied

    @torch.no_grad()
    def _consolidate(self, query, delta, index):
