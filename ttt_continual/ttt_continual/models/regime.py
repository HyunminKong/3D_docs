...
# Frozen in configs/regime_spec.json. Two features beyond the raw rates are
# included because speed and yaw alone cannot tell a straight run from a loop
# that returns to its start: dir_consistency reads how steadily the heading
# changes, arc_over_net how far the path wanders from the chord it spans.
FEATURES = ("speed_mean", "speed_sd", "yaw_mean", "yaw_sd", "still_frac",
            "dir_consistency", "arc_over_net")
...
def describe(c2w, fps=12.0, still_threshold=0.5):
    ...
    unit = delta / (step[:, None] + 1e-8)
    consistency = float((unit[:-1] * unit[1:]).sum(1).mean()) if len(unit) > 1 else 1.0
    net = float(np.linalg.norm(pos[-1] - pos[0]))
    arc = float(step.sum())
    return np.array([
        (step * fps).mean(), (step * fps).std(),
        turn.mean(), turn.std(),
        float((step * fps < still_threshold).mean()),
        consistency,
        arc / max(net, 1e-3),
    ])
