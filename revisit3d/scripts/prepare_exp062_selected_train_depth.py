#!/usr/bin/env python3
"""Register only the preregistered EXP-062 train depths into the RGB frame."""
...
def main() -> None:
    ...
    if not args.confirm_selected_train_depth_registration:
        raise SystemExit("EXP-062 train-depth preparation requires confirmation")

    config_path = Path(args.config)
    config = yaml.safe_load(config_path.read_text())
    output = Path(config["output"]["depth_preparation"])
    if output.exists():
        raise RuntimeError("EXP-062 depth preparation artifact already exists")
    ...
    if not (
        ...
        raise RuntimeError("EXP-062 train-only depth contract failed")
    ...
    history = int(config["data"]["history_frames"])
    frames = sorted(
        {
            (sequence, index)
            for sequence in sequences
            for query in config["data"]["query_frames"]
            for index in range(int(query) - history, int(query) + 1)
        }
    )
    rows = []
    for sequence, index in frames:
        ...
        if not raw.is_file():
            raise RuntimeError(f"EXP-062 raw train depth is missing: {raw}")
