#!/usr/bin/env python3
"""Register only the preregistered EXP-066 train depths into the RGB frame."""
...
def main() -> None:
    ...
    if not args.confirm_selected_train_depth_registration:
        raise SystemExit("EXP-066 train-depth preparation requires confirmation")

    config_path = Path(args.config)
    config = yaml.safe_load(config_path.read_text())
    output = Path(config["output"]["depth_preparation"])
    if output.exists():
        raise RuntimeError("EXP-066 depth preparation artifact already exists")
    ...
    if not (
        ...
        raise RuntimeError("EXP-066 train-only depth contract failed")
    ...
    frames = sorted(
        {
            (sequence, int(query) + int(offset))
            for sequence in sequences
            for query in config["data"]["query_frames"]
            for offset in config["data"]["frame_offsets"]
        }
    )
    ...
    for sequence, index in frames:
        ...
        if not raw.is_file():
            raise RuntimeError(f"EXP-066 raw train depth is missing: {raw}")
