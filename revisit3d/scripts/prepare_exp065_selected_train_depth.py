#!/usr/bin/env python3
"""Register only the preregistered EXP-065 train depths into the RGB frame."""
...
def main() -> None:
    ...
    if not args.confirm_selected_train_depth_registration:
        raise SystemExit("EXP-065 train-depth preparation requires confirmation")

    config_path = Path(args.config)
    config = yaml.safe_load(config_path.read_text())
    output = Path(config["output"]["depth_preparation"])
    if output.exists():
        raise RuntimeError("EXP-065 depth preparation artifact already exists")
    ...
    if not (
        ...
        raise RuntimeError("EXP-065 train-only depth contract failed")
    ...
    frames = sorted(
        {
            ...
            for index in range(int(query) - 4, int(query) + 1)
        }
    )
    rows = []
    for sequence, index in frames:
        ...
        if not raw.is_file():
            raise RuntimeError(f"EXP-065 raw train depth is missing: {raw}")
