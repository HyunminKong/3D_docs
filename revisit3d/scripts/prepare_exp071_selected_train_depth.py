#!/usr/bin/env python3
"""Register only the frozen EXP-071 train depths into the RGB frame."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default="configs/EXP-071_future_revisit_teacher_premise_v11.yaml",
    )
    parser.add_argument(
        "--confirm-selected-train-depth-registration", action="store_true"
    )
    args = parser.parse_args()
    if not args.confirm_selected_train_depth_registration:
        raise SystemExit("EXP-071 train-depth preparation requires confirmation")

    config_path = Path(args.config)
    config = yaml.safe_load(config_path.read_text())
    output = Path(config["output"]["depth_preparation"])
    if output.exists():
        raise RuntimeError("EXP-071 depth preparation artifact already exists")
    ...
    if not (
        config["protocol_revision"] == "v1.1"
        and manifest["role"] == "train"
        ...
        and config["data"]["target_offsets"] == [3, 7, 11]
        and config["data"]["validation_access"] is False
        and config["data"]["terminal_access"] is False
    ):
        raise RuntimeError("EXP-071 train-only depth contract failed")
    ...
    for sequence, index in frames:
        ...
        if not raw.is_file():
            raise RuntimeError(f"EXP-071 raw train depth is missing: {raw}")
        generated = not registered.exists()
        if generated:
            io.imsave(
                registered, register_depth(io.imread(raw)), check_contrast=False
            )
        rows.append(
            {
                "sequence": sequence,
