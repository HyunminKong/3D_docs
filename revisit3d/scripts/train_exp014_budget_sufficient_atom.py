#!/usr/bin/env python3
"""Pre-evidence-justified 1000-step audit of the frozen-key ranked atom."""
...
from revisit3d.experiments.exp012_minimal import run_ranked_utility_episode
...
def _train_steps(head, cache, manifest, indices, config, device, *, seed: int, label: str) -> list[dict]:
    ...
    for step in range(1, total_steps + 1):
        ...
        rollout = run_ranked_utility_episode(
        ...
        if not torch.isfinite(rollout.outer_loss):
            raise RuntimeError(f"non-finite EXP-014 loss at {label}:{step}")
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/EXP-014_budget_sufficient_atom_v10.yaml")
    ...
    if result_path.exists() or checkpoint_path.exists():
        raise RuntimeError("EXP-014 output already exists")
    if config["data"]["split"] != "train" or not torch.cuda.is_available():
        raise RuntimeError("EXP-014 requires train split and CUDA")
    prior = json.loads(Path(config["prior_stage"]).read_text())
    old = json.loads(Path(config["preexisting_budget_evidence"]).read_text())
    if not (
        prior["experiment"] == "EXP-013" and prior["gate"]["passed"] is False
        and prior["gate"]["checks"]["candidate_mean_utility"] is False
        and prior["gate"]["checks"]["candidate_harm"] is False
        and old["selected_steps"] == 1000
        and old["cv_scores"]["1000"]["mean_group_best_valid_utility"]
        > old["cv_scores"]["500"]["mean_group_best_valid_utility"]
        and old["validation_accessed"] is False
    ):
        raise RuntimeError("budget-audit prerequisite contract failed")
    ...
    if not (
        ...
        raise RuntimeError("EXP-014 cache/budget contract failed")
    ...
    for fold, held_out in enumerate(folds):
        ...
        logs = _train_steps(
            ...
            seed=int(config["seed"]) + fold, label=f"budget-fold-{fold}",
        )
    ...
    if sorted(row["index"] for row in oof_rows) != list(range(len(manifest))):
        raise RuntimeError("EXP-014 OOF rows do not form an exact partition")
    ...
    if not all(checks.values()):
        ...
        raise RuntimeError(f"EXP-014 budget audit failed: {checks}")
    ...
    final_logs = _train_steps(
        ...
        seed=int(config["seed"]), label="budget-full-train",
    )
