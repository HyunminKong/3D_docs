#!/usr/bin/env python3
"""Register only EXP-052-selected train depths into the RGB camera frame."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config", default="configs/EXP-052_ttt3r_metric_alignment_premise_v11.yaml"
    )
    ...
    if not args.confirm_selected_train_depth_registration:
        raise SystemExit("EXP-052 train-depth preparation requires confirmation")
    config_path = Path(args.config)
    config = yaml.safe_load(config_path.read_text())
    output = Path(config["output"]["depth_preparation"])
    if output.exists():
        raise RuntimeError("EXP-052 depth preparation artifact already exists")
    ...
    if not (
        ...
        raise RuntimeError("EXP-052 train-only depth contract failed")
    ...
    for sequence, index in frames:
        ...
        if not raw.is_file():
            raise RuntimeError(f"EXP-052 raw train depth is missing: {raw}")
