#!/usr/bin/env python3
"""Minimal current-preservation plus relative-utility ranking atom refit."""
    ...
    for epoch in range(int(training["epochs"])):
        ...
        for index in order:
            ...
            if not torch.isfinite(rollout.outer_loss):
                raise RuntimeError(f"non-finite Stage-0C loss at {label}:{step}")
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/EXP-012_ranked_atom_v12.yaml")
    ...
    if result_path.exists() or checkpoint_path.exists():
        raise RuntimeError("EXP-012 Stage-0C output already exists")
    if config["data"]["split"] != "train" or not torch.cuda.is_available():
        raise RuntimeError("EXP-012 Stage 0C requires train split and CUDA")
    prior = json.loads(Path(config["prior_stage"]).read_text())
    if not (
        prior["protocol_revision"] == "v1.1" and prior["gate"]["passed"] is False
        and prior["gate"]["checks"]["oracle_utility"] is False
        and prior["gate"]["checks"]["candidate_harm"] is False
        and prior["gate"]["checks"]["selection_headroom"] is True
        and prior["validation_accessed"] is False and prior["test_accessed"] is False
    ):
        raise RuntimeError("Stage-0B failure contract changed")
    ...
    if not (
        ...
        raise RuntimeError("EXP-012 Stage-0C cache contract failed")
    ...
    for fold, held_out in enumerate(folds):
        ...
        logs = _train(
            ...
            seed=int(config["seed"]) + fold, label=f"rank-fold-{fold}",
        )
    ...
    if sorted(row["index"] for row in oof_rows) != list(range(len(manifest))):
        raise RuntimeError("Stage-0C OOF rows do not form an exact partition")
    ...
    base_result = {
        "experiment": "EXP-012", "stage": "ranked_minimal_atom_crossfit", "split": "train",
        ...
        "meta_objective_weights": [], "auxiliary_losses": [], "summary": summary,
        "oracle_minus_mean": difference, "gate": {"checks": checks, "passed": all(checks.values())},
    ...
    if not all(checks.values()):
        ...
        raise RuntimeError(f"EXP-012 ranked atom gate failed: {checks}")
    final_head = _new_head(cache, config, device, int(config["seed"]))
    final_logs = _train(
        final_head, cache, manifest, list(range(len(manifest))), config, device,
        seed=int(config["seed"]), label="rank-full-train",
    )
    checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({
        "experiment": "EXP-012", "stage": "ranked_minimal_atom", "split": "train",
        ...
        "meta_objective": base_result["meta_objective"],
